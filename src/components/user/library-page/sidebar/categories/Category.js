...
const Category = () => {
  ...
  const uniqueCategory = [...new Set(bookA.map(item => item.categoryName))];
  const dispatch = useDispatch();

  const handleChange = e => {
    if (!e.target.checked) {
      dispatch(setBook(bookA.filter((book) => book.categoryName.toUpperCase() !== e.target.name.toUpperCase())));
